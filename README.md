...
```mermaid
graph TD
    A[Frontend UI / PDF Upload] -->|Silent Background State| B(Supabase Edge Functions)
    B --> C{Task Type Router}
    C -->|MCQ Request| D[Cyclic Ring 1: 8x Gemini Keys]
    C -->|Summary Request| E[Cyclic Ring 2: 8x Gemini Keys]
    D -->|SSE Text Stream| A
    E -->|SSE Text Stream| A
